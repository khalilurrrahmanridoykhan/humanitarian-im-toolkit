"""Builds the Survey123-dialect version of the rapid site assessment form.

Reuses the same questions and skip logic as build_xlsform.py (the ODK/Kobo
version) but adapts it to the ArcGIS Survey123 XLSForm dialect:

- Adds Esri custom columns (bind::esri:fieldType, bind::esri:fieldLength) so
  Survey123 Connect maps fields to explicit ArcGIS field types/lengths
  instead of guessing.
- Resolves the "one geometry per feature layer" constraint: an ArcGIS
  feature layer has exactly one SHAPE column, so a Survey123 form may use
  only one geo question (geopoint/geotrace/geoshape) as the record's actual
  geometry. `gps_point` is kept as the layer geometry; the optional
  `site_boundary` geoshape gets bind::esri:fieldType explicitly set to
  "null" so Survey123 treats it as a plain attribute instead of a second
  geometry, per Esri's documented workaround for multiple map questions.
- Drops the hint on `begin group` for the protection module — Survey123
  does not render hints on begin group/begin repeat questions (unlike
  ODK/Kobo) — the separate `protection_note` question already carries the
  same guidance, so nothing is lost.

Run: python3 scripts/build_xlsform_survey123.py
Output: forms/rapid_assessment_form_survey123.xlsx
"""

import openpyxl

OUT_PATH = "forms/rapid_assessment_form_survey123.xlsx"
...
SURVEY_HEADER = [
    ...
    "bind::esri:fieldType", "bind::esri:fieldLength",
]

# (type, name, label_en, label_bn, hint_en, required, relevant, constraint,
#  constraint_message, choice_filter, appearance, default, esri_fieldType, esri_fieldLength)
SURVEY_ROWS = [
    ("start", "start", "", "", "", "", "", "", "", "", "", "", "", ""),
    ("end", "end", "", "", "", "", "", "", "", "", "", "", "", ""),
    ("today", "today", "", "", "", "", "", "", "", "", "", "", "", ""),
    ...
    ("text", "enumerator_name", "Enumerator name", "গণনাকারীর নাম", "", "yes", "", "", "", "", "", "", "esriFieldTypeString", "100"),
    ("text", "enumerator_id", "Enumerator ID", "গণনাকারীর আইডি", "", "yes", "", "", "", "", "", "", "esriFieldTypeString", "20"),
    ("date", "assessment_date", "Assessment date", "মূল্যায়নের তারিখ", "", "yes", "", "", "", "", "", "${today}", "esriFieldTypeDate", ""),
    ("text", "site_id", "Site code", "সাইট কোড", "Unique site identifier", "yes", "", "", "", "", "", "", "esriFieldTypeString", "20"),
    ("text", "site_name", "Site name", "সাইটের নাম", "", "yes", "", "", "", "", "", "", "esriFieldTypeString", "150"),
    ("end group", "", "", "", "", "", "", "", "", "", "", "", "", ""),
    ...
    # Primary geometry for the feature layer: bind::esri:fieldType left blank so
    # Survey123 Connect auto-maps it to the layer's SHAPE (point) column.
    ("geopoint", "gps_point", "Site location (GPS)", "সাইটের অবস্থান (জিপিএস)", "Capture at the site's central point", "yes", "", "", "", "", "", "", "", ""),
    # Secondary geo question: explicitly excluded from being the layer geometry.
    ("geoshape", "site_boundary", "Site boundary (optional)", "সাইটের সীমানা (ঐচ্ছিক)", "Trace the outer boundary if mapping the full site", "no", "", "", "", "", "", "", "null", ""),
    ("end group", "", "", "", "", "", "", "", "", "", "", "", "", ""),
    ...
    ("integer", "hh_estimate", "Estimated number of households", "আনুমানিক পরিবারের সংখ্যা", "", "yes", "", ". >= 0", "Must be zero or more", "", "", "", "esriFieldTypeInteger", ""),
    ("integer", "pop_estimate", "Estimated population", "আনুমানিক জনসংখ্যা", "", "yes", "", ". >= ${hh_estimate}", "Population cannot be less than household count", "", "", "", "esriFieldTypeInteger", ""),
    ("end group", "", "", "", "", "", "", "", "", "", "", "", "", ""),
    ...
    ("decimal", "health_facility_distance_km", "Distance to nearest health facility (km)", "নিকটতম স্বাস্থ্যসেবা কেন্দ্রের দূরত্ব (কিমি)", "", "yes", "${health_facility_onsite}='no'", ". >= 0", "Must be zero or more", "", "", "", "esriFieldTypeDouble", ""),
    ...
    # Note: no hint on this begin group (Survey123 doesn't render one) — the
    # protection_note question immediately below carries the same guidance.
    ("begin group", "protection", "Protection", "সুরক্ষা", "", "", "", "", "", "", "", "", "", ""),
    ...
    ("text", "enumerator_remarks", "Enumerator remarks", "গণনাকারীর মন্তব্য", "", "no", "", "", "", "", "multiline", "", "esriFieldTypeString", "1000"),
    ("image", "site_photo", "Site photo (optional)", "সাইটের ছবি (ঐচ্ছিক)", "", "no", "", "", "", "", "", "", "", ""),
    ("end group", "", "", "", "", "", "", "", "", "", "", "", "", ""),
]

# choices and settings are unchanged from the ODK/Kobo version.
